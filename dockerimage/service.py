...
class DefaultHandler(RequestHandler):
    ...
    def get(self):
        self.write({'response': 'Error HUB Operativo', 'status': 200})
...
class ActionHandler(RequestHandler):
    ...
    def otions(self, catalog, action):
        pass
    ...
    def post(self, catalog, action):
        ...
        if auth == False:
            self.write({'response': 'Acceso Denegado', 'status': 400})
        ...
        if (action == 'delete_item'):
            item_id = content['item_id']
            respuesta = delete_item(catalog, item_id)
...
def delete_item(catalog, item_id):
    ...
    if (len(items_to_return)>0):
        ...
        toReturn = {'response':'Elemento eliminado', 'status':200}
    else:
        toReturn = {'response':'Elemento no encontrado', 'status':500}
    return toReturn
...
def make_app():
    urls = [
        ("/", DefaultHandler),
        ("/([^/]+)/([^/]+)", ActionHandler) # http://IP_SERVICIO:5050/catalog/action
    ]
